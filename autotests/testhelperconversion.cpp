...
Q_DECLARE_METATYPE(OklabMatrix)
...
private Q_SLOTS:
    ...
    {
        qRegisterMetaType<OklabMatrix>();
        QTest::addColumn<double>("x");
        QTest::addColumn<double>("y");
        QTest::addColumn<double>("z");
        QTest::addColumn<OklabMatrix>("expectedOklab");
    ...
    void testFromXyzd65ToOklab()
    {
        ...
        QFETCH(OklabMatrix, expectedOklab);
        const auto actualOklab = fromXyzd65ToOklab(QGenericMatrix<1, 3, double>(initArray));
        constexpr double epsilon = 0.001;
        QVERIFY(isNearlyEqual(actualOklab(0, 0), expectedOklab(0, 0), epsilon));
        QVERIFY(isNearlyEqual(actualOklab(1, 0), expectedOklab(1, 0), epsilon));
        QVERIFY(isNearlyEqual(actualOklab(2, 0), expectedOklab(2, 0), epsilon));
    }

    void testFromCmscielabD50ToOklab_data()
    {
        qRegisterMetaType<OklabMatrix>();
        QTest::addColumn<cmsCIELab>("originalCmscielab");
        QTest::addColumn<OklabMatrix>("expectedOklab");
    ...
    {
        QFETCH(cmsCIELab, originalCmscielab);
        QFETCH(OklabMatrix, expectedOklab);
        const auto actualOklab = fromCmscielabD50ToOklab(originalCmscielab);
        constexpr double epsilon = 0.001;
        QVERIFY(isNearlyEqual(actualOklab.L, expectedOklab(0, 0), epsilon));
        QVERIFY(isNearlyEqual(actualOklab.a, expectedOklab(1, 0), epsilon));
        QVERIFY(isNearlyEqual(actualOklab.b, expectedOklab(2, 0), epsilon));
    }
};
